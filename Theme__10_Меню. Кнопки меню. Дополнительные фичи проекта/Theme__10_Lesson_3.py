...
class Snake:  # создаем класс змейка
    ...
    def change_color(self):
        colors = ["White", "Black", "Green", "Cyan", "Grey", "Pink"]
...
while is_game_active:
    ...
    # ----------------->FPS TEXT<------------------------#
    fps_text = font2.render("SPEED " + str(fps), True, "Yellow")
    screen.blit(fps_text, (WIDTH - 170, 5))  # отрисовываем текст
    ...
    if is_eat:  # если еда съедена (кубик подобран)
        snake.change_color()
        fps = 10 + snake.count // 2 # увеличиваем скорость змейки
        is_repeat = True # повторно ищем новые координаты для еды

        colors = ["White", "Black", "Green", "Cyan", "Grey", "Pink"]
        randColor = random.choice(colors)
        food_color = randColor
        while is_repeat:
            is_repeat = False
    ...
    pygame.draw.rect(screen, food_color, (food_x, food_y, size, size))  # создаем новый кубик (еда)
